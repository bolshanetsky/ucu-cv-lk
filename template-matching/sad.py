...
def template_matching_sad(frame, template):
    '''
    SAD template matching algorithm.
    '''
    ...
    for dy in range(0, heigth - heigth_t):
        for dx in range(0, width - width_t):
            diff = np.abs(frame[dy:dy + heigth_t, dx:dx + width_t] - template)
            score[dy, dx] = diff.sum()

    point = np.unravel_index(score.argmin(), score.shape)
...
def main():
    ...
    for frame in test_images:
        ...
        pt = template_matching_sad(gray, template)
